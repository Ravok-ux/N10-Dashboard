...
function _html() {
  ...
  <div style="display:flex;flex-direction:column;height:100%;gap:0;padding:0">
    ...
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;padding:14px 20px;flex-shrink:0">
      ${[
        ["rst-k-notas",    "NOTAS ACTIVAS",   "📋"],
        ["rst-k-vencidas", "VENCIDAS",         "⚠️"],
        ["rst-k-capital",  "CAPITAL PENDIENTE","💵"],
        ["rst-k-interes",  "INTERÉS GENERADO", "📈"],
        ["rst-k-total",    "TOTAL A PAGAR",    "💰"],
      ].map(([id,lbl,ico]) => `
        <div style="background:var(--c-surface);border:1px solid var(--c-border);border-radius:10px;padding:14px 16px">
          <div style="font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">${ico} ${lbl}</div>
          <div style="font-size:17px;font-weight:800;color:var(--c-text);font-variant-numeric:tabular-nums" id="${id}">–</div>
        </div>`).join("")}
    </div>
    ...
    <div style="padding:0 20px 10px;flex-shrink:0;display:flex;gap:6px;flex-wrap:wrap">
      ${FILTROS.map(f => `
        <button data-rst-f="${f}" onclick="RemisionesUI.setFiltro('${f}')"
          style="padding:5px 12px;border-radius:20px;border:1px solid var(--c-border);
      ...
      <div style="background:var(--c-surface);border:1px solid var(--c-border);border-radius:10px;overflow:hidden">
        <div style="overflow-x:auto">
          <table style="width:100%;border-collapse:collapse;font-size:11.5px">
            ...
            <tbody id="rst-tbody">
              <tr><td colspan="12" style="padding:32px;text-align:center;color:#9CA3AF">Cargando…</td></tr>
            </tbody>
          </table>
        </div>
      </div>
  ...
  <div id="rst-modal-hist" class="modal-overlay hidden" onclick="if(event.target===this)RemisionesUI.cerrarHistorial()">
    <div class="modal" style="max-width:780px;width:96vw">
      ...
      <div id="rst-hist-resumen" style="background:var(--c-surface2,rgba(255,255,255,.04));border:1px solid var(--c-border);
      ...
      <div id="rst-hist-vacio" style="display:none;padding:32px;text-align:center;color:#9CA3AF;font-size:13px">
  ...
  <div id="rst-modal-abono" class="modal-overlay hidden">
    <div class="modal" style="max-width:440px">
      ...
      <div id="rst-abono-info" style="background:var(--c-surface2,rgba(255,255,255,.04));border-radius:8px;
        padding:12px 14px;margin-bottom:16px;font-size:12px;line-height:1.7;border:1px solid var(--c-border)">
      </div>
      ...
      <div id="rst-abono-preview" style="font-size:12px;color:#9CA3AF;margin-bottom:12px"></div>
...
const _th = (align = "left") =>
  `padding:10px 14px;text-align:${align};font-weight:700;color:#9CA3AF;font-size:10px;
  ...
  window.RemisionesUI = {
    setFiltro(f) {
      ...
      document.querySelectorAll("[data-rst-f]").forEach(b => {
        ...
        b.style.background = activo ? "var(--c-text)" : "transparent";
        b.style.color      = activo ? "var(--c-surface)" : "#9CA3AF";
        b.style.borderColor= activo ? "var(--c-text)" : "var(--c-border)";
      });
      _renderTabla();
    },
    ...
    abrirHistorial(id) {
      ...
      if (resEl) resEl.innerHTML = [
        ...
        `<div><span style="color:#9CA3AF">${lbl}</span><br>${val}</div>`
      ).join("");
      ...
      if (!abonos.length) {
        ...
        tbody.innerHTML = abonos.map((ab, idx) => {
          ...
          return `<tr style="border-bottom:1px solid var(--c-border)">
            <td style="padding:8px 14px;color:#9CA3AF;font-size:10px">${idx + 1}</td>
            <td style="padding:8px 14px;white-space:nowrap;color:#9CA3AF">
              ${fmtDia(ab.fecha)}</td>
            ...
            <td style="padding:8px 14px;text-align:right;font-variant-numeric:tabular-nums;color:var(--c-text)">
            ...
            <td style="padding:8px 14px;font-family:monospace;font-size:10px;color:#9CA3AF">
              ${esc(ab.recibo || "–")}</td>
...
function _renderTabla() {
  ...
  if (lista.length === 0) {
    tbody.innerHTML = `<tr><td colspan="12" style="padding:32px;text-align:center;color:#9CA3AF">
  ...
  tbody.innerHTML = lista.map(r => {
    ...
    return `<tr style="border-bottom:1px solid var(--c-border);${pagado ? "opacity:.6" : ""}">
      ...
      <td style="padding:9px 14px;color:#9CA3AF">${esc(r.ingenieroAlias || "–")}</td>
      ...
      <td style="padding:9px 14px;text-align:right;font-variant-numeric:tabular-nums;color:#9CA3AF">
      ...
      <td style="padding:9px 14px;text-align:center;font-weight:700;
        ...
        ${mora > 0 ? mora + " días" : "–"}</td>
      <td style="padding:9px 14px;text-align:center;color:#9CA3AF;white-space:nowrap">
